...
def process_gene_info(gene_info, record_id, start_adjust, end_adjust, species, gene_symbol):
    ...
    start_position = start - start_adjust
    end_position = end + end_adjust

    # Calculate total user-specified sequence length in bp (need to add 1)
    sequence_length = end_position - start_position + 1

    print(f"Specified region: {accession_number}:{start_position}-{end_position}")
    print(f"Specified region length: {sequence_length}bp")
    print("")

    return accession_number, start_position, end_position, sequence_length


# Function #3 - get list of genes and features in specified region
def get_genes_in_region(accession, start, end):
    ...
    return genes_in_region, collected_features
...
def run(
    ...
    accession_number, start_position, end_position, sequence_length = process_gene_info(
    ...
    genes, collected_features = get_genes_in_region(accession_number, start_position, end_position)
